...
(
 ...
 //suppose  ip_rx_data in the first period is only 32-bits valid
...
reg  [63:0]              ip_rx_data_d0 ;
reg  [63:0]              ip_rx_data_d1 ;
...
parameter REC_HEADER       =  5'b00010  ;
//parameter REC_HEADER1      =  5'b00100  ;
parameter REC_DATA         =  5'b01000  ;
parameter REC_END          =  5'b10000  ;

(*mark_debug = "ture"*)reg [4:0]     state      ;
reg [4:0]     next_state ;

assign ip_rx_ready = (state == IDLE);
...
begin
    case(state)
	 IDLE            :  begin
	                    if (ip_rx_req == 1'b1)
						  next_state <= REC_HEADER ;
						else
						  next_state <= IDLE ;
						end
	 REC_HEADER    :	begin
                        if (ip_rx_cnt == header_length[5:3]  )
							next_state <= REC_DATA ;
						else
	                        next_state <= REC_HEADER ;
						end
	 REC_DATA       :   begin
                        if (ip_checksum_error || ip_rx_end || ip_addr_check_error)
							next_state <= REC_END ;		
...
begin
  if (~rst_n)
    ip_rx_end <= 1'b0 ;
  else if (state == REC_DATA && ip_rx_cnt == ip_total_data_length[15:3] - 1)
    ip_rx_end <= 1'b1 ;
  else
    ip_rx_end <= 1'b0 ;
end
...
begin
  if (~rst_n)
    udp_rx_req <= 1'b0 ;
  else if (state == REC_HEADER  && ip_rx_cnt == 1)
    udp_rx_req <= 1'b1 ;
  else
    udp_rx_req <= 1'b0 ;
end
...
begin
  if (~rst_n)
    ip_rx_cnt <= 16'd0 ;
  else if (state == REC_HEADER || state == REC_DATA)
...
begin
  if (~rst_n)
    header_length_buf <= 4'd5 ;
  else if (state == REC_HEADER && ip_rx_cnt == 16'd0)
    header_length_buf <= ip_rx_data[26:24] ;
end
...
begin
  if (~rst_n)
    ...
    ip_rec_data_length <= ip_rx_data[15:0]  ;
...
begin
  if (~rst_n)
    net_protocol <= 8'd0 ;
  else if (state == REC_HEADER && ip_rx_cnt == 16'd1)
    net_protocol <= ip_rx_data[23:16] ;
end
...
begin
  ...
  else if (state == REC_HEADER && ip_rx_cnt == 16'd2)
    begin    
        ip_rec_source_addr <= ip_rx_data[63:32] ;
        ip_rec_destination_addr <= ip_rx_data[31:0];
    end
  else
    begin
